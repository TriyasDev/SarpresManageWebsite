...
@push('styles')
{{-- style sama persis seperti semula --}}
<style>
    .bar-canvas-wrap { position: relative; width: 100%; height: 200px; }
    @media (min-width: 640px)  { .bar-canvas-wrap { height: 220px; } }
    @media (min-width: 1024px) { .bar-canvas-wrap { height: 240px; } }

    .donut-canvas-wrap { position: relative; width: 180px; height: 180px; flex-shrink: 0; }
    ...
    .month-pills-absolute { position: relative; width: 100%; height: 40px; margin-top: 8px; }
    .month-pill { position: absolute; transform: translateX(-50%); white-space: nowrap; cursor: default; transition: all 0.1s ease; }
</style>
@endpush
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (barCanvas) {
            ...
            options: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        ticks: {
                            stepSize: Math.ceil(Math.max(...BAR_DATA, 1) / 5) || 1,
        ...
        setTimeout(() => { if (barChart) alignMonthPills(barChart); }, 100);
        window.addEventListener('resize', () => { setTimeout(() => { if (barChart) alignMonthPills(barChart); }, 200); });
    }

    // Donut chart
    const donutCanvas = document.getElementById('donutChart');
    if (donutCanvas && DONUT_DATA.length) {
